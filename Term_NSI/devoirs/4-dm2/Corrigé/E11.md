...
* Les `if cond: pass; else action()` s'éliminent facilement. On teste la condition contraire et on obtient `if not(cond): action()`.
...
def nb_couples_anagrammes(taille: int, phrase: str) -> int:
...
print(nb_couples_anagrammes(taille, phrase))
```
...
**Exercice 1** : Reprendre la classe `ABR`, ou bien `Tas`, et ajouter sans doublon les mots. Nous n'avons plus besoin de `set` ici.

**Exercice 2** : De même, construire une structure (sans doublon de signature) avec des tuples `(signature, effectif)`. Pour chaque nouveau mot unique, trouver sa signature, le chercher dans votre structure. S'il est absent, ajouter un nœud `(signature, 1)`. S'il est présent, incrémenter son effectif. Nous n'avons plus besoin de `dict` ici.
