...
* Il vaut mieux débuter les paramètres de la fonction par la taille, puis la liste.
* Aérer `ecart>maximum`, et ajouter un accent.
* Pour la lecture, utiliser plutôt `map(int, input().split())`.
...
def grand_écart(taille: int, tableau: list[int]) -> int:
